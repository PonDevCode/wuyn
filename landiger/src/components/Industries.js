...
const data = [
  {
    ...
    bullets: ['Khách tự đặt lịch theo liệu trình và kỹ thuật viên', 'Nhắc lịch và nhắc liệu trình tiếp theo qua Zalo', 'Quản lý thẻ liệu trình, combo và tiền cọc', 'Doanh thu theo từng dịch vụ, từng nhân viên'],
  },
  {
    key: 'salon', tab: 'Salon tóc', title: 'Lĩnh vực Salon tóc', tint: '#F3EFFD', icon: icons.salon,
    chip: 'Nhắc quay lại: 18 khách', head: 'Lịch thợ hôm nay',
    rows: [['10:00', 'Thợ Minh · Cắt + nhuộm', 'Đã đặt'], ['11:30', 'Thợ Hằng · Uốn phồng', 'Đã đặt'], ['14:00', 'Thợ Minh · Cắt nam', 'Trống']],
    bullets: ['Khách đặt lịch theo thợ, xem giờ trống', 'Lưu lịch sử kiểu tóc, màu nhuộm từng khách', 'Tự tính hoa hồng cho thợ', 'Nhắc khách quay lại sau 4–6 tuần'],
  },
  {
    key: 'fit', tab: 'Fitness & Yoga', title: 'Lĩnh vực Fitness & Yoga', tint: '#EEF4FD', icon: icons.fitness,
    chip: 'Sắp hết gói: 7 học viên', head: 'Lớp hôm nay',
    rows: [['06:30', 'Yoga cơ bản · 12/15 chỗ', 'Còn chỗ'], ['18:00', 'HIIT · 20/20 chỗ', 'Đã đầy'], ['19:30', 'Pilates · 8/12 chỗ', 'Còn chỗ']],
    bullets: ['Học viên đăng ký lớp, giữ chỗ online', 'Quản lý gói tập và số buổi còn lại', 'Check-in học viên, lịch huấn luyện viên', 'Nhắc gia hạn khi gói sắp hết'],
  },
  {
    key: 'clinic', tab: 'Phòng khám', title: 'Lĩnh vực Phòng khám', tint: '#EBF8F3', icon: icons.clinic,
    chip: 'Nhắc tái khám: 24 bệnh nhân', head: 'Lịch khám sáng nay',
    rows: [['08:00', 'BS. An · Nha khoa', 'Đã đến'], ['09:15', 'BS. Bình · Da liễu', 'Đã xác nhận'], ['10:30', 'BS. An · Nha khoa', 'Chờ xác nhận']],
    bullets: ['Đặt lịch khám theo bác sĩ, chuyên khoa', 'Hồ sơ bệnh nhân và lịch sử khám', 'Tự động nhắc tái khám', 'Thu phí, tiền cọc và báo cáo doanh thu'],
  },
  {
    key: 'edu', tab: 'Giáo dục', title: 'Lĩnh vực Giáo dục', tint: '#FDF6E6', icon: icons.education,
    chip: 'Form tuyển sinh: đang mở', head: 'Đăng ký mới tuần này',
    rows: [['T2', 'Anh Tuấn · IELTS 6.5', 'Đã tư vấn'], ['T3', 'Chị Ngọc · Giao tiếp', 'Đã đóng phí'], ['T4', 'Bạn Khoa · IELTS 7.0', 'Mới']],
    bullets: ['Trang tuyển sinh và form đăng ký học', 'Quản lý lớp, học viên và lịch học', 'Nhắc học phí, lịch học qua Zalo', 'Theo dõi tỷ lệ đăng ký theo từng kênh'],
  },
  {
    key: 'fnb', tab: 'F&B', title: 'Lĩnh vực F&B – Nhà hàng', tint: '#FDEEEB', icon: icons.fnb,
    chip: 'Khách thân thiết: 312 thành viên', head: 'Đặt bàn tối nay',
    rows: [['18:30', 'Bàn 4 người · Anh Duy', 'Đã cọc'], ['19:00', 'Bàn 8 người · Công ty A', 'Đã xác nhận'], ['19:45', 'Bàn 2 người · Chị My', 'Mới']],
    bullets: ['Website menu và đặt bàn online', 'Nhận đơn mang đi, thanh toán trước', 'Chương trình khách thân thiết', 'Báo cáo món bán chạy theo ngày'],
  },
];
  ...
  return (
    <section id="theo-nganh" className="relative overflow-hidden px-4 pb-16 pt-14 sm:px-8 lg:pb-[54px] lg:pt-16">
      ...
      >
              ...
              <div className="absolute inset-x-4 top-4 flex flex-col gap-3 rounded-[18px] bg-white p-4 shadow-[0_24px_40px_-24px_rgba(11,20,36,0.35)] sm:inset-x-10 sm:top-10 sm:p-[18px]">
                ...
                {cur.rows.map(([a, b, c]) => (
                  <div key={a} className="flex items-center gap-3 rounded-xl bg-page px-3 py-[11px] text-[13px]">
                    ...
                    <span className="shrink-0 rounded-md bg-tint px-2 py-[3px] text-[11px] font-bold text-brand">{c}</span>
                  </div>
                ))}
              </div>
        ...
        >
          <div className="text-[22px] font-extrabold sm:text-[28px]">{cur.title}</div>
          <ul className="m-0 flex list-none flex-col gap-3 p-0">
            {cur.bullets.map((b) => (
              <li key={b} className="flex items-start gap-3 text-[15px] leading-normal text-[#2A3348] sm:text-base">
                <span className="mt-px flex size-[22px] shrink-0 items-center justify-center rounded-[7px] bg-tint">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M5 12.5l4.5 4.5L19.5 6.5" stroke="#004BEC" strokeWidth="3.4" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </span>
                {b}
              </li>
